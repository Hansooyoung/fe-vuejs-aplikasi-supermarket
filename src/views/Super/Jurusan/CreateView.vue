...
// Form state
const formData = ref({
  nama_jurusan: '', // Change this to 'nama_jurusan'
}); 
...
const handleSubmit = async () => {
  try {
    ...
    const response = await api.request({
      method: 'POST',
      url: '/jurusan', // Update API endpoint to '/jurusan'
      headers: {
    ...
    router.push({ name: 'jurusan-super' }); // Adjust the redirect to the correct route
  } catch (error) {
  ...
  <DefaultLayoutAdmin>
    <div class="grid">
      <div class="flex flex-col gap-9">
        <!-- Form Start -->
        <DefaultCard cardTitle="Tambah Jurusan">
          <form @submit.prevent="handleSubmit">
            <div class="p-6.5">
              <!-- Input for Jurusan Nama -->
              <InputGroup
                label="Nama Jurusan"
                type="text"
