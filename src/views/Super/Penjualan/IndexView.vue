...
const fetchDataPenjualan = async () => {
  try {
    const response = await api.request({
      method: 'GET',
      url: `/penjualan-super?page=${currentPage.value}&search=${searchQuery.value}&sort=${sortField.value}&direction=${sortDirection.value}`,
      headers: {
...
const confirmDelete = (penjualan) => {
  Swal.fire({
    ...
    text: `Anda akan menghapus transaksi penjualan dengan ID ${penjualan.id}!`,
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`/penjualan/${penjualan.id}`)
        Swal.fire(
          'Dihapus!',
          'Transaksi penjualan telah dihapus.',
          'success'
        )
        fetchDataPenjualan() // Refresh data
      } catch (error) {
        console.error('Error deleting penjualan:', error)
        Swal.fire(
          'Gagal!',
          'Gagal menghapus transaksi penjualan.',
          'error'
        )
      }
    }
