...
const handleSearchMember = () => {
  ...
  // Jika input kosong, reset member list
  if (!searchQuery.value) {
    memberList.value = []
    return
  }
  
  // Jika input adalah angka dan kurang dari 10 digit, tampilkan error
...
// Ubah fetchMemberData untuk menangani validasi
...
const handleSubmit = async () => {
  ...
  const confirmResult = await Swal.fire({
    ...
    html: `
      <div class="text-left">
        <p><strong>Total Belanja:</strong> Rp${totalHarga.value.toLocaleString('id-ID')}</p>
        <p><strong>Tunai:</strong> Rp${formData.value.tunai.toLocaleString('id-ID')}</p>
        <p><strong>Kembalian:</strong> Rp${kembalian.value.toLocaleString('id-ID')}</p>
      </div>
    `,
    icon: 'question',
  ...
  try {
    ...
    // Tambahkan tunai dan kembalian ke data yang dikirim
    const requestData = {
      ...formData.value,
    ...
    // Tampilkan dialog sukses dengan opsi cetak atau selesai
    // Tampilkan dialog sukses dengan opsi cetak atau selesai
const result = await Swal.fire({
  ...
  html: `
    <div class="text-center">
      ...
      <p>Tunai: Rp${formData.value.tunai.toLocaleString('id-ID')}</p>
      <p>Kembalian: Rp${kembalian.value.toLocaleString('id-ID')}</p>
    </div>
  `,
  showCancelButton: true,
  confirmButtonText: 'Cetak Struk',
  cancelButtonText: 'Selesai',
  confirmButtonColor: '#3085d6', // Warna biru untuk tombol Cetak
  cancelButtonColor: '#6c757d',  // Warna abu-abu untuk tombol Selesai
  focusConfirm: false,
  allowOutsideClick: false,
  customClass: {
    confirmButton: 'swal-confirm-button', // Class khusus untuk tombol confirm
    cancelButton: 'swal-cancel-button'   // Class khusus untuk tombol cancel
  }
});

    // Jika user memilih cetak, panggil endpoint cetak struk
// Di bagian handleSubmit, perbaiki bagian cetak struk:
if (result.isConfirmed) {
  try {
    const printResponse = await api.request({
      method: 'POST',
      url: `/penjualan/${response.data.penjualan.id}/cetak-struk`,
      data: {
    ...
    Swal.fire('Sukses', 'Struk berhasil dikirim ke printer', 'success');
  } catch (printError) {
    console.error('Gagal mencetak struk:', printError);
    
    let errorMsg = 'Gagal mencetak struk';
    if (printError.response?.data?.error) {
      errorMsg += ': ' + printError.response.data.error;
    }
    ...
    Swal.fire({
      ...
      html: `
        <div class="text-left">
          <p>${errorMsg}</p>
          ${printError.response?.data?.printer_debug ? 
            `<p class="mt-2 text-sm">${printError.response.data.printer_debug}</p>` : ''}
        </div>
      `,
    ...
    // Redirect ke halaman penjualan setelah cetak/selesai
...
<DefaultCard cardTitle="Member">
  <div class="p-4">
    ...
    <div v-if="selectedMember" class="mt-3 bg-white p-3 rounded-lg shadow-sm space-y-2 text-sm">
