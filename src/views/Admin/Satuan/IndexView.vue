...
const itemToDelete = ref(null) // Menyimpan Satuan barang yang akan dihapus
...
const fetchDataSatuan = async () => {
  try {
    const response = await api.request({
      method: 'GET',
      url: `/satuan?page=${currentPage.value}&search=${searchQuery.value}`,
      headers: {
    ...
    satuanData.value = response.data.data.data
...
const deleteItem = async () => {
  ...
  try {
    await api.request({
      method: 'DELETE',
      url: `/satuan/${itemToDelete.value.id}`,
      headers: {
        Accept: 'application/json',
        'Content-type': 'application/json'
      }
...
const confirmDelete = (satuan) => {
  itemToDelete.value = satuan
  modalMessage.value = 'Apakah Anda yakin ingin menghapus Satuan barang ini?'
...
const exportToPDF = async () => {
  try {
    ...
    link.download = 'laporan-satuan.pdf'
    link.click()
...
const exportToExcel = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'laporan-satuan.xlsx'
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
...
<template>
  <DefaultLayoutAdmin>
    ...
    >
      <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Satuan Barang</h4>
      <!-- Action Buttons -->
      ...
      <div class="max-w-full overflow-x-auto">
        <div class="flex justify-between mb-4">
          <div class="flex gap-4 items-center">
            <button
              @click="$router.push({ name: 'satuan.create' })"
        ...
        <table class="w-full table-auto">
          ...
          <tbody>
            ...
            <tr v-for="(satuan, index) in satuanData" :key="index">
              ...
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ satuan.nama_satuan }}</p>
              </td>
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  ...
                  <button @click="confirmDelete(satuan)" class="hover:text-primary">
                    Delete
                  </button>
                </div>
              </td>
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
        <h3 class="text-xl font-semibold mb-4">Import Satuan Barang</h3>
