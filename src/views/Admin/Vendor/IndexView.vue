...
const itemToDelete = ref(null) // Menyimpan Vendor barang yang akan dihapus
...
const fetchDataVendor = async () => {
  try {
    const response = await api.request({
      method: 'GET',
      url: `/vendor?page=${currentPage.value}&search=${searchQuery.value}`,
      headers: {
    ...
    vendorData.value = response.data.data.data
    totalPages.value = response.data.data.last_page // Total halaman dari API
  } catch (error) {
    console.error('Error fetching Vendor barang:', error)
  }
}
...
const confirmDelete = (vendor) => {
  ...
  modalMessage.value = 'Apakah Anda yakin ingin menghapus Vendor barang ini?'
...
const exportToPDF = async () => {
  try {
    ...
    link.download = 'laporan-vendor.pdf'
    link.click()
...
const exportToExcel = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'laporan-vendor.xlsx'
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
...
<template>
  <DefaultLayoutAdmin>
    ...
    >
      <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Vendor Barang</h4>
      <!-- Action Buttons -->
            ...
            <tr v-for="(vendor, index) in vendorData" :key="index">
              ...
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  ...
                  <button @click="confirmDelete(vendor)" class="hover:text-primary">
                    Delete
                  </button>
                </div>
              </td>
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
        <h3 class="text-xl font-semibold mb-4">Import Vendor Barang</h3>
