...
const itemToDelete = ref(null) // Menyimpan voucher yang akan dihapus
...
const fetchDataVoucher = async () => {
  try {
    const response = await api.request({
      method: 'GET',
      url: `/voucher?page=${currentPage.value}&search=${searchQuery.value}`,
      headers: {
    ...
    voucherData.value = response.data.data.data
...
const deleteItem = async () => {
  ...
  try {
    await api.request({
      method: 'DELETE',
      url: `/voucher/${itemToDelete.value.id}`,
      headers: {
        Accept: 'application/json',
        'Content-type': 'application/json'
      }
...
const confirmDelete = (voucher) => {
  itemToDelete.value = voucher
  modalMessage.value = 'Apakah Anda yakin ingin menghapus voucher ini?'
...
const exportToPDF = async () => {
  try {
    ...
    link.download = 'laporan-voucher.pdf'
    link.click()
...
const exportToExcel = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'laporan-voucher.xlsx'
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
...
<template>
  <DefaultLayoutAdmin>
    ...
    >
      <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Data Voucher</h4>
      <!-- Action Buttons -->
        ...
        <table class="w-full table-auto">
          ...
          <tbody>
            ...
            <tr v-for="(voucher, index) in voucherData" :key="index">
              ...
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ voucher.nama_voucher }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ voucher.harga_point }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ voucher.jenis_voucher }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ voucher.status }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ voucher.nilai_voucher }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ voucher.min_pembelian }}</p>
              </td>
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  ...
                  <button @click="confirmDelete(voucher)" class="hover:text-primary">
                    Delete
                  </button>
                </div>
              </td>
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
        <h3 class="text-xl font-semibold mb-4">Import Voucher</h3>
