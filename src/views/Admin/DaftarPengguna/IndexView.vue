...
const itemToDelete = ref(null) // Menyimpan Users barang yang akan dihapus
...
const fetchDataUsers = async () => {
  try {
    const response = await api.request({
      method: 'GET',
      url: `/users?page=${currentPage.value}&search=${searchQuery.value}`,
      headers: {
    ...
    usersData.value = response.data.data.data
    totalPages.value = response.data.data.last_page // Total halaman dari API
  } catch (error) {
    console.error('Error fetching Users barang:', error)
  }
}
...
const deleteItem = async () => {
  ...
  try {
    await api.request({
      method: 'DELETE',
      url: `/users/${itemToDelete.value.id}`,
      headers: {
        Accept: 'application/json',
        'Content-type': 'application/json'
      }
    ...
    Swal.fire({
      title: 'Berhasil!',
      text: 'Data users berhasil dihapus.',
      icon: 'success',
...
const confirmDelete = (users) => {
  itemToDelete.value = users
  modalMessage.value = 'Apakah Anda yakin ingin menghapus Users barang ini?'
...
const exportToPDF = async () => {
  try {
    ...
    link.download = 'laporan-users.pdf'
    link.click()
...
const exportToExcel = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'laporan-users.xlsx'
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
  ...
  <DefaultLayoutAdmin>
          ...
          <tbody>
            <tr v-if="usersData.length === 0">
              <td colspan="5" class="text-center py-5 px-4 text-gray-500">
                Data users tidak ada
              </td>
            </tr>
            <tr v-for="(users, index) in usersData" :key="index">
              ...
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ users.nama }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ users.email }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ users.role }}</p>
              </td>
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  ...
                  <button @click="confirmDelete(users)" class="hover:text-primary">
                    Delete
                  </button>
                </div>
              </td>
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
        <h3 class="text-xl font-semibold mb-4">Import Users Barang</h3>
