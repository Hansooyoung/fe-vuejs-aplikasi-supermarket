...
const itemToDelete = ref(null) // Menyimpan Diskon barang yang akan dihapus
...
const fetchDataDiskon = async () => {
  try {
    const response = await api.request({
      method: 'GET',
      url: `/diskon?page=${currentPage.value}&search=${searchQuery.value}`,
      headers: {
    ...
    diskonData.value = response.data.data.data
    totalPages.value = response.data.data.last_page // Total halaman dari API
  } catch (error) {
    console.error('Error fetching Diskon barang:', error)
  }
}
...
const deleteItem = async () => {
  ...
  try {
    await api.request({
      method: 'DELETE',
      url: `/diskon/${itemToDelete.value.id}`,
      headers: {
        Accept: 'application/json',
        'Content-type': 'application/json'
      }
    ...
    Swal.fire({
      title: 'Berhasil!',
      text: 'Data diskon berhasil dihapus.',
      icon: 'success',
...
const confirmDelete = (diskon) => {
  itemToDelete.value = diskon
  modalMessage.value = 'Apakah Anda yakin ingin menghapus Diskon barang ini?'
...
const exportToPDF = async () => {
  try {
    ...
    link.download = 'laporan-diskon.pdf'
    link.click()
...
const exportToExcel = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'laporan-diskon.xlsx'
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
...
<template>
  <DefaultLayoutAdmin>
    ...
    >
      <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Diskon Barang</h4>
      <!-- Action Buttons -->
      ...
      <div class="max-w-full overflow-x-auto">
        <div class="flex justify-between mb-4">
          <div class="flex gap-4 items-center">
            <button
              @click="$router.push({ name: 'diskon.create' })"
        ...
        <table class="w-full table-auto">
          ...
          <tbody>
            <tr v-if="diskonData.length === 0">
              <td colspan="7" class="text-center py-5 px-4 text-gray-500">
                Data diskon tidak ada
              </td>
            </tr>
            <tr v-for="(diskon, index) in diskonData" :key="index">
              ...
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ diskon.nama_diskon }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ diskon.jenis_diskon }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ diskon.nilai_diskon }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ diskon.tanggal_mulai }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ diskon.tanggal_berakhir }}</p>
              </td>
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  ...
                  <button @click="confirmDelete(diskon)" class="hover:text-primary">
                    Delete
                  </button>
                </div>
              </td>
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
        <h3 class="text-xl font-semibold mb-4">Import Diskon Barang</h3>
