...
const fetchVendorData = async () => {
  try {
    ...
    vendorList.value = response.data.data.data
  } catch (error) {
...
const addBarang = (barang) => {
  if (!selectedBarang.value.some((b) => b.barcode === barang.barcode)) {
    selectedBarang.value.push({
      ...
      harga_beli: Number(barang.harga_beli) || 0, // Ensure it's a number and default to 0
      jumlah: 1,
      stok: barang.stok || 0
    })
  }
...
const handleSubmit = async () => {
  ...
  const confirmResult = await Swal.fire({
    ...
    confirmButtonText: 'Ya, Simpan Pembelian',
  ...
  try {
    errors.value = []
    formData.value.barang = selectedBarang.value.map(({ barcode, harga_beli, jumlah }) => ({ barcode, harga_beli, jumlah }))
