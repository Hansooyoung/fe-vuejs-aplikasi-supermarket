...
const fetchDataDetail = async () => {
  ...
  try {
    ...
    tanggalMasukInput.value = pembelian.value.tanggal_masuk === "Belum Masuk" ? "" : pembelian.value.tanggal_masuk;
    
    detailData.value = response.data.detail_pembelian.data; 
  } catch (error) {
...
const updateTanggalMasuk = async () => {
  ...
  try {
    const response = await api.request({
      method: 'PUT',
      url: `/pembelian/${pembelian.value.id}/tanggal-masuk`,
  ...
  <DefaultLayoutUser>
    ...
    >
      ...
      <div class="mb-6 p-4 border border-gray-300 rounded-md bg-gray-50 dark:bg-meta-4">
        ...
        <p class="text-lg text-black dark:text-white"><strong>Total Pembelian:</strong> Rp{{ pembelian?.total }}</p>
      </div>
        ...
        <table class="w-full table-auto">
          ...
          <tbody>
            <tr v-for="(detail, index) in detailData" :key="index">
              ...
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ detail.harga_beli }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ detail.jumlah }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ detail.sub_total }}</p>
              </td>
            </tr>
          </tbody>
        </table>
