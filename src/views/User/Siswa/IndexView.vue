...
const deleteItem = async () => {
  ...
  try {
    ...
    if (error.response?.status === 400) {
      modalMessage.value = 'Siswa masih dmeminjam.';
...
const confirmDelete = (siswa) => {
  itemToDelete.value = siswa;
  modalMessage.value = 'Apakah Anda yakin ingin menghapus siswa ini?';
            ...
            <tr v-for="(siswa, index) in siswaData" :key="index">
              ...
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  <button @click="$router.push({ name: 'siswa.update', params: { id: siswa.id } })" class="hover:text-primary ">Edit</button>
                  <button @click="confirmDelete(siswa)" class="hover:text-primary">Delete</button>
                </div>
              </td>
            </tr>
          </tbody>
